...
Repository repository = new Repository("workers.txt");
    ...
    if (int.TryParse(input, out int operation))
    {
        switch (operation)
        {
            //Просмотр записей
            case 1:
                PrintWorkersTable(repository.GetAllWorkers());
                break;
            //Просмотр записи по ID
            case 2:
                ViewRecordById(repository);
                break;
            ...
            case 5:
                ViewByDateRange(repository);
                break;
            //Сортировка
            case 6:
                SortWorker(repository);
                break;
            ...
            //Генерация записей
            case 8:
                GenerateRecordsWorker(repository);
                break;
            //Выход
            case 0:
...
//Просмотр записи по ID
static void ViewRecordById(Repository repository)
{
...
//Загрузка записей в диапазоне дат
static void ViewByDateRange(Repository repository)
{
    Console.Write("Введите начальную дату (дд.мм.гггг): ");

    string inputDateFrom = Console.ReadLine();

    if (!DateTime.TryParseExact(inputDateFrom, "dd.MM.yyyy", null, System.Globalization.DateTimeStyles.None, out DateTime dateFrom))
    {
        Console.ForegroundColor = ConsoleColor.Red;
    ...
    string inputDateTo = Console.ReadLine();

    if (!DateTime.TryParseExact(inputDateTo, "dd.MM.yyyy", null, System.Globalization.DateTimeStyles.None, out DateTime dateTo))
    {
        Console.ForegroundColor = ConsoleColor.Red;
        Console.WriteLine("Некорректный формат даты. Используйте формат дд.мм.гггг.");
        Console.ResetColor();
...
//Сортировка
static void SortWorker(Repository repository)
{
    ...
    switch (field)
    {
        ...
        default:
            ...
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine("Некорректный номер поля для сортировки.");
            Console.ResetColor();
...
//Генерация записей
static void GenerateRecordsWorker(Repository repository)
{
